...
if (chat && chat.antispam) {
    ...
    if (isSticker && !m.isAdmin) {
        if (m.sender in conn.spamSticker) {
            ...
            let timeSinceLastSpam = m.messageTimestamp  - conn.spamSticker[m.sender].lastspam;
            ...
            if (timeSinceLastSpam <= 10) {
                ...
                conn.spamSticker[m.sender].lastspam = m.messageTimestamp 
            }
        } else {
            conn.spamSticker[m.sender] = {
                count: 1,
                lastspam: m.messageTimestamp
            };
        }
    }
    ...
    if (isText && !m.isAdmin) {
        if (m.sender in conn.spamText) {
            ...
            let timeSinceLastSpam = m.messageTimestamp  - conn.spamText[m.sender].lastspam;
            ...
            if (timeSinceLastSpam <= 15) {
                ...
                conn.spamText[m.sender].lastspam = m.messageTimestamp 
            }
        } else {
