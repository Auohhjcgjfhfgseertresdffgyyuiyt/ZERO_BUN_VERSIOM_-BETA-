...
async function ciciai(question) {
  ...
  if (!chat) throw new Error("CICI tidak mengembalikan jawaban")
...
let handler = async (m, { text, usedPrefix, command }) => {
  if (!text)
    return m.reply(
      "Example:\n${usedPrefix + command} jelaskan javascript"
    )

  await m.reply(loading.wait)

  try {
    const res = await ciciai(text)
...
━━━━━━━━━━━━━━━`
    ...
    console.error("CICI ERROR:", e)
    m.reply("❌ Cici lagi error, coba lagi nanti.")
  }
}

handler.help = ["cici <pertanyaan>"]
handler.tags = ["ai"]
handler.command = ["cici","cii"]
handler.limit = true
