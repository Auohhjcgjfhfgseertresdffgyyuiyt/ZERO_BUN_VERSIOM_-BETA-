...
async function ihancer(buffer, {
    ...
    if (!sizes.includes(size)) throw new Error(`Available sizes: ${sizes.join(', ')}`);
    ...
    // Bun punya File sendiri
    form.append(
        "file",
        new File([buffer], `rynn_${Date.now()}.jpg`, { type: "image/jpeg" })
    );
    ...
    const res = await fetch("https://ihancer.com/api/enhance", {
        ...
        headers: {
            ...
            // host tidak perlu, Bun otomatis set
            ...form.headers   // Bun 1.1+ mendukung multipart auto-header
        }
    });

    if (!res.ok) {
        const errTxt = await res.text().catch(() => "");
        throw new Error(`IHancer Error: ${res.status} ${res.statusText} - ${errTxt}`);
    }
