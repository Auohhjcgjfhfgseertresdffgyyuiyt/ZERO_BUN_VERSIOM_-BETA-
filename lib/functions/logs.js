  ...
  if (budy && m.key.remoteJid !== "status@broadcast") {
    if (print) {
      console.log(
        ...
        chalk.blue(`${m.pushName}`), //nama users
...
function saveUserData(data, obj) {
  data.push(obj);
  if (data.length > 10) data.shift(); // Hapus item tertua
  fs.writeFileSync('./database/logs.json', JSON.stringify(data, null, 2));
}
